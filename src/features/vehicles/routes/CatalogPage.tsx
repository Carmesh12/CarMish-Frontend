...
function getCardStatusBadge(vehicle: Vehicle): {
  labelKey: string;
  fallback: string;
  variant:
    | "default"
  ...
  if (vehicle.availabilityStatus !== "AVAILABLE") {
    return {
      labelKey: `vehicles.${vehicle.availabilityStatus.toLowerCase()}`,
      fallback: vehicle.availabilityStatus,
      variant:
  ...
  return {
    labelKey: `listingType.${vehicle.listingType}`,
    fallback: vehicle.listingType,
    variant: "gold",
  };
}
...
export function CatalogPage() {
  ...
  const sortBy =
    (searchParams.get("sortBy") as VehicleQueryParams["sortBy"]) || "createdAt";
  const sortOrder =
    (searchParams.get("sortOrder") as VehicleQueryParams["sortOrder"]) ||
    "desc";
  ...
  async function toggleFavorite(vehicleId: string) {
    ...
    try {
      ...
      notifySuccess(
        res.isFavorited
          ? t("catalog.favoriteAdded", "Added to favorites")
          : t("catalog.favoriteRemoved", "Removed from favorites"),
      );
    } catch {
  ...
  const localizedFuelOptions = FUEL_OPTIONS.map((o) =>
    o.value
      ? { value: o.value, label: t(`fuel.${o.value}`, o.value) }
      : { value: "", label: t("catalog.allFuelTypes", "All") },
  );
  const localizedTransmissionOptions = TRANSMISSION_OPTIONS.map((o) =>
    o.value
      ? { value: o.value, label: t(`transmission.${o.value}`, o.value) }
      : { value: "", label: t("catalog.allTransmissions", "All") },
  );
  const localizedListingTypeOptions = LISTING_TYPE_OPTIONS.map((o) =>
    o.value
      ? { value: o.value, label: t(`listingType.${o.value}`, o.value) }
      : { value: "", label: t("catalog.allListingTypes", "All") },
  );
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-12 pb-8">
          <h1 className="text-4xl sm:text-5xl font-bold mb-3">
            <span className="text-gradient-gold">
              {t("catalog.title", "Vehicle Catalog")}
            </span>
          </h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <form onSubmit={handleSearch} className="flex gap-3 mb-6">
          <div className="relative flex-1">
            <Search
              className="absolute left-3.5 rtl:left-auto rtl:right-3.5 top-1/2 -translate-y-1/2 text-mesh-muted"
              size={18}
            />
            <input
              type="text"
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="text-mesh-muted hover:text-mesh-text transition-colors cursor-pointer"
                >
                  <X size={20} />
                </button>
              </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <p className="text-sm text-mesh-muted">
            {loading
              ...
              : t("catalog.resultsCount", "{{count}} vehicles found", {
        ...
        {loading ? (
          <div className="flex justify-center py-20">
            <Spinner
              size={48}
              label={t("catalog.loadingVehicles", "Loading vehicles...")}
            />
          </div>
        ) : vehicles.length === 0 ? (
          <EmptyState
              ...
              {vehicles.map((vehicle) => {
                ...
                const priceLabel =
                  vehicle.listingType === "RENT"
                    ...
                    : "";
                const statusBadge = getCardStatusBadge(vehicle);
                    ...
                    >
                      ...
                      <div className="absolute top-3 left-3 rtl:left-auto rtl:right-3">
                        <Badge variant={statusBadge.variant}>
                          {t(statusBadge.labelKey, statusBadge.fallback)}
                        </Badge>
                      </div>
                    </Link>
                    ...
                    <div className="flex flex-col flex-1 p-4">
                      ...
                      <p className="text-lg font-bold text-mesh-gold mb-2">
                        {showPrice
                          ...
                          : t("catalog.contactForPrice", "Contact for price")}
                        {showPrice && priceLabel && (
                          <span className="text-sm font-normal text-mesh-muted">
                            {priceLabel}
                          </span>
                        )}
                      </p>
                    ...
                    {isAuthenticated && role === "USER" && (
                      <div className="px-4 pb-4">
                        ...
                        >
                          ...
                          <span
                            className={
                              isFav ? "text-red-400" : "text-mesh-muted"
                            }
                          >
                            {isFav
                              ? t("catalog.favorited", "Favorited")
                              : t("catalog.addFavorite", "Favorite")}
                          </span>
                        </button>
                      </div>
                    )}
