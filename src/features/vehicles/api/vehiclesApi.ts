...
export const vehiclesApi = {
  ...
  getById: (id: string) => authJson<Vehicle>(`/vehicles/${id}`),

  getImages: (vehicleId: string) =>
    authJson<VehicleImage[]>(`/vehicles/${vehicleId}/images`),

  getReviews: (vehicleId: string, page = 1, limit = 10) =>
    authJson<ReviewListResponse>(
      `/vehicles/${vehicleId}/reviews?page=${page}&limit=${limit}`,
    ),
  ...
  archive: (id: string) =>
    authJson<Vehicle>(`/vehicles/${id}`, { method: "DELETE" }),

  getMyVehicles: () => authJson<Vehicle[]>("/vehicles/my"),

  uploadImages: (vehicleId: string, files: File[]) => {
    const formData = new FormData();
    files.forEach((f) => formData.append("images", f));
    return authFormData<VehicleImage[]>(
