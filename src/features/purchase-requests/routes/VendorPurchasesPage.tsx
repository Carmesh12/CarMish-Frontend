...
export function VendorPurchasesPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    purchaseRequestsApi.getVendorRequests()
  ...
  const handleAction = async (id: string, status: 'APPROVED' | 'REJECTED') => {
    setActionId(id);
    try {
      const updated = await purchaseRequestsApi.updateStatus(id, status);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-mesh-text">{t('purchase.incomingRequests')}</h1>
              ...
              <tbody>
                {requests.map((req) => (
                  <tr key={req.id} className="border-b border-mesh-border/50 hover:bg-mesh-surface/50 transition-colors">
                    ...
                    <td className="p-4 text-mesh-muted">{req.userId.slice(0, 8)}…</td>
                    ...
                    <td className="p-4 text-mesh-muted">{new Date(req.createdAt).toLocaleDateString()}</td>
                    <td className="p-4">
                      {req.status === 'PENDING' ? (
