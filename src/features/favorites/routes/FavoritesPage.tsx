...
type FavoriteItem = Awaited<
  ReturnType<typeof favoritesApi.getMyFavorites>
>[number];

function getFavoriteCardStatusBadge(vehicle: FavoriteItem["vehicle"]): {
  labelKey: string;
  fallback: string;
  variant:
    | "default"
  ...
  if (vehicle.listingType === "SALE") {
    return { labelKey: "vehicles.forSale", fallback: "SALE", variant: "info" };
  }

  if (vehicle.listingType === "RENT") {
    return {
      labelKey: "vehicles.forRent",
      fallback: "RENT",
      variant: "warning",
    };
  }
      ...
      {items.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((fav) => {
            ...
            const statusBadge = getFavoriteCardStatusBadge(v);
